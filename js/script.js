function send_mail() {
	window.location.href =
		"mailto:seth@goldin.io?subject=Personal%20Inquiry%20" +
		encodeURI(document.getElementById("name_field").value) +
		"&body=" +
		encodeURI(document.getElementById("message_field").value);
	return false;
}

var w = (neuron_canvas.width = window.innerWidth),
	h = (neuron_canvas.height = window.innerHeight),
	ctx = neuron_canvas.getContext("2d"),
	opts = {
		range: 180,
		baseConnections: 3,
		addedConnections: 5,
...
window.addEventListener("resize", function () {
	opts.vanishPoint.x = (w = neuron_canvas.width = window.innerWidth) / 2;
	opts.vanishPoint.y = (h = neuron_canvas.height = window.innerHeight) / 2;
	ctx.fillRect(0, 0, w, h);
});
